...
namespace Graph {
    ...
    bool EdgeList<V, E>::containEdge(const E &edge) const {
        return std::find_if(_edges->begin(), _edges->end(), [& edge] (const Edge & e) -> bool
            { return edge == e.edgeName; }
        ) != _edges->end();
    }

    template<typename V, typename E>
    bool EdgeList<V, E>::containVertex(const V &vertex) const {
        return std::find_if(_vertices->begin(), _vertices->end(), [& vertex] (const V & v) -> bool {
            return vertex == v;
        }) != _vertices->end();
    }
